...
def core_set(infile, preset, coverage, delta, outname, maf):
    print("Python Version Check...")
    ver_check()
    
    infile = infile
    pfile = preset
    Coverage = np.float(coverage)
    ...
    Coreset_file_name = output + "_Coreset.csv"
    Maf_file_name = output + "_Remove_markers.csv"
    Core_sample_name = output + "_CoreSample_list.txt"
    ...
    t_s_time = time.time()
    mpe=range(1, np.sum(var_num-1,dtype=int)+1)
    for idx in mpe:
        print(idx, "-th iteration starts at", now())
        i_s_time = time.time()
        ...
        #print("candidate :", ', '.join([x[0] +'(' + str(x[1]) + ' index)' for x in list(zip(candidate_counts_names,candidate_counts_idx))]))
        ...
        #print("Selected", len(select), "candidate genes for", idx, '('+', '.join([x for x in select_name])+')')
        if len(select) != 1:
            ...
            select = select[0]
            #select = np.random.choice(select,size=1)
            select_name = candidate_counts_names[select]
        ...
        print("Final selected sample is", select_name[0])   
        print("Total selected samples are", len(result_idx)) 
        print("Coverage is", str(round(np.float(coverage[2]),6)), "%")
        print("Difference is ", str(round(np.float(coverage[3]),6)), "%")
        ...
        print("Iteration time", np_dt[0], np_dt[1])
        print('-'*50)
        if np.float(coverage[2]) >= Coverage or np.float(coverage[3]) < delta:
            t_e_time = time.time()
            np_dt = dt(t_e_time - t_s_time)
            print("Core selection completed!")
            print("Total running time", np_dt[0], np_dt[1])
            print("GenoCore selects ",len(result_idx), "element for core sets")
            ...
            print("Making Core sample list file...")
            for sname in result:
