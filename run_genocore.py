...
def parse_my_command_line():
    ...
    # VCFtoCSV
    opt_parser['VCFtoCSV'].usage = "Usage: python run_genocore.py VCFtoCSV -i [VCF file] -o [Output name] -p [Y or N (phased)] -g [Y or N (gziped)]\n    Help: python run_genocore.py VCFtoCSV -h"
    ...
    # GenoCore
    opt_parser['CoreSet'].usage = "Usage: python run_genocore.py CoreSet -i [CSV file] -p [Preset txt] -c [Coverage rate] -d [Covergence rate], -o [Output name], -m [MAF rate] \n    Help: python run_genocore.py CoreSet -h"
    ...
    opt_parser['CoreSet'].add_option('-p', '--predefined', default = None, type = 'str', dest='preset', action='store',
        help = 'Preset text file (one sample per line')
    ...
    # SelectVCF
    opt_parser['SelectVCF'].usage = "Usage: python run_genocore.py SelectVCF -i [VCF file] -s [Sample list] -o [Output name] \n    Help: python run_genocore.py VCFtoCSV -h"
    ...
    try:
        ...
        parser.usage += '  This prgram works only Python version 3 and requires Numpy library.\n'
        parser.usage += '  To work this program, select Methods, VCFtoCSV or CoreSet.\n\n'
...
if __name__ == '__main__':
    ...
    if args[0] == 'SelectVCF':
        ...
        sample = opt.sample
        outname = opt.output_name
        result = VCF_select(infile, sample, outname)
        if result == "Done":
